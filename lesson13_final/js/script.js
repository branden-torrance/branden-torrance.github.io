...
// "date" const taken from --Globale Constants-- section at the top of the document
...
if (page.textContent == "Home") {
    ...
    .then((weatherStats) => {
        ...
        document.querySelector("#weatherDesc").textContent = 
            weatherStats.current.weather[0].description.replace(/(^\w{1})|(\s+\w{1})/g,(first) => first.toUpperCase());

        // Temperature
        document.querySelector("#temperature").textContent = weatherStats.current.temp.toFixed(0)

        // Humidity
        document.querySelector("#humidity").textContent = weatherStats.current.humidity

        // Wind Speed
        document.querySelector("#windspeed").textContent = weatherStats.current.wind_speed.toFixed(0)
        ...
        if (typeof(weatherStats.alerts) != "undefined") {
            document.getElementById("alert_name").textContent = weatherStats.alerts.event;
            document.getElementById("alert_description").textContent = weatherStats.alerts.description;
    ...
    // 3-Day Forecast for Battle Creek, MI (Still within IF statement from Weather Summary above)
        ...
        // Display Temperature
        document.querySelector("#forecastTemp1").textContent = forecast1.temp.day.toFixed(0);
        document.querySelector("#forecastTemp2").textContent = forecast2.temp.day.toFixed(0);
        document.querySelector("#forecastTemp3").textContent = forecast3.temp.day.toFixed(0);

        // Display Day
        document.querySelector("#forecastDay1").textContent = dayOfWeek[index1];
        document.querySelector("#forecastDay2").textContent = dayOfWeek[index2];
        document.querySelector("#forecastDay3").textContent = dayOfWeek[index3];
        ...
        document.querySelector(`#forecastImage1`).setAttribute('alt', weatherDesc1);
        document.querySelector(`#forecastImage2`).setAttribute('alt', weatherDesc2);
        document.querySelector(`#forecastImage3`).setAttribute('alt', weatherDesc3);
